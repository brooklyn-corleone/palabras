...
// Пометка «это слово не собирается из букв». Не то же самое, что `flagged`: с самим
// словом и переводом всё в порядке, ломается именно головоломка — ответ из нескольких
// слов, слишком длинный набор фишек, или в подсказке проскочил сам ответ.
// Слово остаётся в колоде и во всех остальных форматах, режим letters его больше не берёт.
//
// Как и flagged, не трогает `edited` и не входит в SEED_FIELDS: жалоба на формат — не правка
// слова, пополнение колоды должно продолжать чинить его текст и пометку не затирать.
export function skipLetters(id, on) {
  const w = wordById(id);
  if (!w) return;
  w.noLetters = !!on;
  w.updated = now();
